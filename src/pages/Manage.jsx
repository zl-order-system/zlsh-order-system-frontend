    ...
    let handleButtonClick = (event, method, index) => {
        ...
        if ( ( o[index]["selectData"]["acceptClickButton"] && method == "PATCH" ) || ( o[index]["selectData"]["acceptTrashClickButton"] && method == "DELETE" ) || method == "POST" ){
            ...
            postOrder(JSON.stringify(data), method).then( res => {
                ...
                // Caution: Temporary fix before project refactoring is done
                location.reload();
            }).catch( error => {
                setLoaderState("hidden")
                if(method == "POST"){
        ...
        return (
            <div className=" flex flex-col w-full my-[0.75rem]" key={index}>
                ...
                <div className={`flex flex-row justify-between items-center`}>
                    ...
                    <div className={`flex flex-row gap-3 ${element["css"]["displayOfTrashButton"]}`}>
                        ...
                        <button onClick={(event) => { handleButtonClick(event, "DELETE", index) }} className={`block w-4`}><img src={trashIcon}></img></button>
                    </div>
                </div>
            </div>
        );
