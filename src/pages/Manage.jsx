    ...
    value["item"].forEach((element, index) => {
        ...
        switch (element["stateOfPreviewText"]) {
            ...
            case "未訂餐":
                ...
                value["item"][index]["mealName"] = value["item"][index]["allMealNumber"][0]
                value["item"][index]["lunchBox"] = "自備餐盒"
                break;
            default:
                break;
        }
...
function Items({itemsData}){
    ...
    let handleSelectChange = (event, index, value) => {
        ...
        if(value=="box"){
            ...
            o[index]["mealName"]= event.target.value
        }
        setNewItemData(o)
        if(o[index]["mealName"] != originData["item"][index]["mealName"] || o[index]["lunchBox"] != originData["item"][index]["lunchBox"]){
            ...
            setNewItemData(o)
        }else if(o[index]["mealName"] == originData["item"][index]["mealName"] || o[index]["lunchBox"] == originData["item"][index]["lunchBox"]){
            ...
            <div className=" flex flex-col w-full my-[0.75rem]" key={index}>
                ...
                <div className={`flex flex-row justify-between items-center`}>
                    ...
                    <div className={`${element["css"]["displayOfSelectBlock"]} flex flex-row gap-2 mt-[2px]`}>
                        ...
                        <select defaultValue={element["mealName"]} onChange={(event)=>{handleSelectChange(event,index,"num")}} className="py-0.5 px-1 text-[0.9rem] text-[#6C6C6C] font-[300] rounded-[0.25rem] border-[1px] border-[#ACACAC] leading-[100%] bg-white">
                    ...
                    <button onClick={(event) => {handleButtonClick(event, index)}} className={`text-[#35B1E2] text-[1rem] font-[600] ${element["css"]["displayOfOrderButton"]} ${element["css"]["opacityOfButton"]} ${element["css"]["cursorOfButton"]}`}>預定</button>
