...
function createData(value) {
    ...
    value["bodyData"].forEach((element, index) => {
        let elementValue = {
            ...
            displayOfTrashButton : null,
            acceptTrashClickButton : null
        }
        switch (element["state"]) {
            case "已繳費":
                ...
                elementValue.displayOfTrashButton = "hidden"
                break;
            case "未繳費":
                elementValue.displayOfOrderButton = "hidden"
                ...
                elementValue.displayOfTrashButton = "block"
                elementValue.acceptTrashClickButton = true
                break;
            case "未訂餐":
        ...
        value["bodyData"][index]["css"] = {
            ...
            "displayOfTrashButton" : elementValue.displayOfTrashButton
        };
        value["bodyData"][index]["selectData"] = {
            "acceptClickButton": elementValue.acceptClickButton,
            "acceptTrashClickButton" : elementValue.acceptTrashClickButton
        };
    });
    return returnData;
}
...
function Items({ itemsData, setData, setLoaderState, setSuccessHintState, setErrorHintState }) {
    ...
    let handleButtonClick = (event, method, index) => {
        let o = JSON.parse(JSON.stringify(newItemData))
        if (o[index]["selectData"]["acceptClickButton"] == true || o[index]["selectData"]["acceptTrashClickButton"] == true) {
            o[index]["selectData"]["acceptClickButton"] = false
            o[index]["css"]["opacityOfButton"] = "opacity-50"
            o[index]["css"]["cursorOfButton"] = "cursor-default"
            setNewItemData(o)
